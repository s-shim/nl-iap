...
model.setParam('LogFile', 'log/log_difficulty%s_NL_%s_lambda%s_%s_%s.txt'%(difficulty,machineName,int(logSum*100), dataID, rep))
...
for i in G.nodes():
    ...
    for stb in theStable:
        ...
        model.addConstr(LinExpr(LHS_ub)<=rhs_ub, name='Eq.UB(%s,%s)'%(i,n_stb))
        model.addConstr(LinExpr(LHS_lb)>=rhs_lb, name='Eq.LB(%s,%s)'%(i,n_stb))
...
result.to_csv(r'result/resultMIS_NL_lambda%s_%s_%s_Difficulty%s.csv'%(int(logSum*100),dataID,rep,difficulty), index = False)#Check
    
optSol = pd.DataFrame(list(zip(varName,varValue)),columns =['varName','varValue'])
optSol.to_csv(r'log/optMIS_NL_lambda%s_%s_%s_Difficulty%s.csv'%(int(logSum * 100),dataID,rep,difficulty), index = False)#Check
